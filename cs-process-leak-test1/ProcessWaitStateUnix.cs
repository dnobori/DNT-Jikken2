    ...
    internal sealed class ProcessWaitState : IDisposable
    {
        ...
        internal void ReleaseRef()
        {
            ...
            lock (waitStates)
            {
                ...
                System.Diagnostics.Debug.Assert(foundState);
                if (foundState)
                {
        ...
        {
            System.Diagnostics.Debug.Assert(processId >= 0);
        ...
        public void Dispose()
        {
            System.Diagnostics.Debug.Assert(!Monitor.IsEntered(_gate));
        ...
        internal DateTime ExitTime
        {
            get
            {
                lock (_gate)
                {
                    System.Diagnostics.Debug.Assert(_exited);
                    return _exitTime;
                }
            }
        }
        ...
        private void CheckForNonChildExit()
        {
            ...
            if (!_isChild)
            {
                ...
                else // error from kill
                {
                    ...
                    {
                        System.Diagnostics.Debug.Fail("Unexpected errno value from kill");
                        exited = true;
                    }
                }
                if (exited)
            ...
            else
            {
                ...
                while (true)
                {
                    ...
                    int remainingTimeout = millisecondsTimeout == Timeout.Infinite ?
                        Timeout.Infinite :
                        (int)Math.Max(millisecondsTimeout - ((System.Diagnostics.Stopwatch.GetTimestamp() - startTime) / (double)System.Diagnostics.Stopwatch.Frequency * 1000), 0);
        ...
        {
            System.Diagnostics.Debug.Assert(Monitor.IsEntered(_gate));
            System.Diagnostics.Debug.Assert(_waitInProgress == null);
            System.Diagnostics.Debug.Assert(!_isChild);
